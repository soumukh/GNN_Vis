...
TASK MAP (high-level)
----------------------
This file is organized into named TASK sections to make it easy for
contributors to find, modify, and reason about the code. Each TASK is
annotated with an explanatory header (purpose, inputs, outputs, side-effects)
so you can make targeted changes without wading through unrelated logic.

Tasks:
    1) Imports & Dependencies     - external libraries and local utilities
    2) Configuration             - warnings, paths, constants
    3) Stability Cache helpers   - file-based caches for stability results
    4) Server-side caches        - in-memory caches for packages/datasets/models
    5) Device setup              - torch device selection
    6) Model definitions         - `GCNNet`, `GATNet`, and link heads
    7) Data Loading & Model I/O  - load precomputed packages, instantiate models
    8) Explanation & Inference   - helpers to run model inference and explainers
    9) Stability Backends        - node/link stability compute routines
 10) Link Stability Cache      - parquet/pickle cache helpers
 11) Visualization helpers     - subgraph extraction and cytoscape rendering
 12) Dash app layout & callbacks - UI wiring (bottom of file)

When editing, keep each TASK focused and avoid mixing responsibilities.
If you split logic into new files, update the TASK MAP above and add
import stubs at the top.
"""
...
# Explanation helpers moved to app_expl.py
from app_expl import (
    run_inference,
    explain_link_prediction,
    run_cpa_iv,
    _path_fingerprints,
)
...
# Outputs: module-level constants like `MODEL_DIR`, `DATA_DIR`, `STABILITY_CACHE_DIR`
# Side-effects: creates `stability_cache` directory if missing
...
# --- TASK: Global Variables & Setup ---
# Purpose: declare global constants and simple look-up lists used by the app.
# Inputs: none
# Outputs: `MODEL_DIR`, `DATA_DIR`, `DEFAULT_*`, `AVAILABLE_*`
# Notes: Keep these lightweight; larger configuration should be moved to a
#       dedicated config file (e.g., `config.py`) when the project grows.
...
# Purpose: determine absolute paths for models, data and other artifacts.
# Inputs: current script location (`__file__`) or CWD when running interactively.
# Outputs: `SCRIPT_DIR`, `MODEL_ABS_DIR`, `DATA_ABS_DIR`
# Notes: If workspace layout changes, update these derivations accordingly.
...
# --- TASK: Stability Cache (file-backed) ---
# Purpose: read/write stability precomputations to disk to avoid expensive
#          re-computation during interactive exploration.
# Inputs: cache key (dataset/model/sigma), results list
# Outputs: pickle/parquet files under `stability_cache/`
# Side-effects: creates `stability_cache` dir if missing; prints logging
...
# Purpose: in-memory caches to avoid repeated disk reads and model instantiation.
# Inputs: package/dataset identifiers
# Outputs: returns cached objects or loads & caches them on-demand
# Notes: these caches are process-local; consider redis/memcached for multi-process
...
def get_attention_weights(package_meta):
    package = get_cached_package_if_missing(package_meta)
    if isinstance(package, dict):
        return package.get('attention_weights')
    return None
...
# --- TASK: Device setup ---
# Purpose: choose compute device (CPU/GPU) and expose as module-level `device`.
# Inputs: environment GPU availability via `torch.cuda.is_available()`
# Outputs: `device` variable used across tensor operations and model placement
# Notes: For testing or CI, you can override to CPU by setting `device = torch.device('cpu')`
...
# Purpose: define model classes used by the app (`GCNNet`, `GATNet`), including
#          optional link-prediction heads and `predict_links()` helpers.
# Inputs: tensor inputs `x`, `edge_index`; initialization arguments
# Outputs: model instances compatible with `instantiate_model()`
# Notes: Keep model code framework-agnostic where possible. If adding new models,
#        add them here and update `instantiate_model()` accordingly.
...
# --- TASK: Data loading & model instantiation ---
# Purpose: load precomputed model packages, convert saved state_dicts to tensors,
#          instantiate model classes and move to `device`.
# Inputs: `package` metadata, `model_type`, `num_features`, `num_classes`
# Outputs: PyTorch `model` ready for inference
# Notes: `package` is expected to be a dict with `model_state_dict` and init args.
...
# --- TASK: Explanation & Inference ---
# Purpose: provide safe wrappers around model forward passes and explainer
#          invocations (GNNExplainer, link explainers, CPA-IV helper).
# Inputs: model, data, node/edge indices
# Outputs: JSON-serializable dicts with predictions, confidences, explanations
# Notes: Keep these functions deterministic when possible (seed RNGs) and
#        avoid returning non-serializable objects from callbacks.
...
# Purpose: read/write per-edge stability results (parquet preferred, pkl fallback).
# Inputs: cache_key (dataset/model/sigma), results list
# Outputs: DataFrame/parquet or legacy pickle files on disk
# Notes: Precompute scripts drop parquet here for fast UI loads.
...
# Purpose: compute per-edge stability S(u,v) by running GNNExplainer for the
#          target edge on clean and noisy inputs; returns per-edge metrics.
# Inputs: model, data, sigma, sample_size
# Outputs: list of dicts per edge with `source`, `target`, `stability`, ...
# Notes: This is expensive; use caching and sampling to keep interactive latency low.
...
# Purpose: fast approximation of stability by running `n_trials` noisy
#          forward passes and measuring prediction consistency per node.
# Inputs: model, data, predictions_data, sigma, sample_size, n_trials
# Outputs: list of per-node dicts (stability, entropy, embedding variance)
# Notes: Much faster than per-node explainer; suitable for interactive UIs.
...
# Purpose: helper functions to normalize and fingerprint CPA (causal path)
#          outputs so they are comparable and easily highlighted in the UI.
# Inputs: lists/dicts describing paths
# Outputs: hashable fingerprints or transformed path data

# --- Path Utilities (used by CPA-IV link explanation) ---

# Path utilities moved to app_expl.py


# --- TASK: Visualization helpers ---
# Purpose: construct subgraph views, Cytoscape node/edge payloads, and
#          extract visualization-focused attributes (colors, sizes, highlights).
# Inputs: `data`, `center_node_idx`, `explanation`, `cpa_data`, `link_predictions`
# Outputs: lists/dicts suitable for Dash Cytoscape and Plotly rendering
# Notes: Keep visualization logic UI-focused; avoid embedding heavy compute.

# --- Visualization Helpers ---

def create_subgraph_cytoscape(data, center_node_idx, predictions, class_to_color_map, explanation, cpa_data, link_predictions=None, analysis_mode='node_classification', selected_path_idx=None, color_mode='predicted', true_labels=None):
    ...
    for node_id in nodes_in_subgraph:
        ...
        if color_mode == 'actual' and true_labels is not None and node_id < len(true_labels):
            try:
                actual_idx = int(true_labels[node_id])
                node_color = class_to_color_map.get(actual_idx, node_color)
            except Exception:
                pass
        elif color_mode == 'correctness':
            if true_labels is not None and node_id < len(true_labels):
                try:
                    actual_idx = int(true_labels[node_id])
                    is_correct = int(predictions[node_id]) == actual_idx
                    node_color = '#2ecc71' if is_correct else '#e74c3c'
                except Exception:
                    pass
            else:
                node_color = class_to_color_map.get(pred_class_idx, node_color)
...
def plot_embeddings(embeddings_2d, predictions, true_labels, selected_node_id, class_to_color_map, num_classes, target_node_id=None):
    ...
    # Color points by TRUE label (stringified for stable mapping)
    df['True Label'] = np.array(true_labels).astype(str)

    category_orders = {"True Label": [str(i) for i in range(num_classes)]}

    fig = px.scatter(df, x='Dim 1', y='Dim 2', color='True Label',
                     hover_data=['Node ID', 'Predicted Class'], custom_data=['Node ID'],
...
def get_one_hop_neighbors(pyg_data, node_idx):
    """Return the 1-hop neighbors of a node from the graph edge list."""
    try:
        if not hasattr(pyg_data, 'edge_index') or pyg_data.edge_index is None:
            return []
        neighbors = set()
        for src, dst in pyg_data.edge_index.t().tolist():
            if int(src) == int(node_idx):
                neighbors.add(int(dst))
            elif int(dst) == int(node_idx):
...
def plot_neighbor_importance(node_importance, node_idx, preds=None, y_labels=None, one_hop_neighbors=None):
    """Show a table of 1-hop neighbors with their true class, predicted class, and importance."""
    if one_hop_neighbors is None:
        one_hop_neighbors = sorted(node_importance.keys())
    ...
    for neighbor in one_hop_neighbors:
        if neighbor not in node_importance:
            importance = 0.0
        else:
            importance = float(node_importance.get(neighbor, 0.0))

        # Get true class if labels are available
        true_class = 'N/A'
        if y_labels is not None and int(neighbor) < len(y_labels):
            try:
                true_class = int(y_labels[int(neighbor)])
            except Exception:
                true_class = 'N/A'

        # Get predicted class if predictions are available
        predicted_class = 'N/A'
        if preds is not None and int(neighbor) < len(preds):
            try:
                predicted_class = int(preds[int(neighbor)])
            except Exception:
                predicted_class = 'N/A'
    ...
    neighbor_list = sorted(neighbor_list, key=lambda item: item['Importance'], reverse=True)

    fig = go.Figure(data=[go.Table(
        header=dict(
            values=['Neighbor', 'True Class', 'Predicted Class', 'Importance'],
            fill_color='#2c3e50',
        ...
        cells=dict(
            values=[
                [str(item['Neighbor']) for item in neighbor_list],
                [str(item['True Class']) for item in neighbor_list],
                [str(item['Predicted Class']) for item in neighbor_list],
                [f"{item['Importance']:.3f}" for item in neighbor_list],
            ],
            fill_color='white',
            align='left',
    ...
    fig.update_layout(
        title=f'1-Hop Neighbor Details for Node {node_idx}',
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)',
    ...
    fig.update_traces(
        columnwidth=[0.22, 0.26, 0.26, 0.26]
    )
...
def plot_feature_importance(feature_importance, node_idx, top_k=20):
    ...
    fig = px.bar(
        ...
        x='Feature',
        y='Importance',
        title=f'Top Feature Importance for Node {node_idx}',
        text_auto='.3f',
        color='Importance',
...
def _build_attention_neighbor_bar(attention_weights, node_idx, layer_name='conv1', top_k=None):
    """Build a bar chart of 1-hop attention scores for the selected node and layer."""
    try:
        layer_data = attention_weights.get(layer_name)
        if not layer_data or not isinstance(layer_data, (list, tuple)) or len(layer_data) < 2:
            return None

        edge_index, att_values = layer_data
        if edge_index is None or att_values is None:
            return None

        edge_arr = np.array(edge_index)
        att_arr = np.array(att_values)

        # Support both shapes: (2, E) or (E, 2)
        if edge_arr.ndim == 2 and edge_arr.shape[0] == 2:
            srcs, dsts = edge_arr[0], edge_arr[1]
        elif edge_arr.ndim == 2 and edge_arr.shape[1] == 2:
            srcs, dsts = edge_arr[:, 0], edge_arr[:, 1]
        else:
            return None

        # Collapse per-head scores if present
        if att_arr.ndim == 2:
            att_scores = np.mean(att_arr, axis=1)
        else:
            att_scores = att_arr.astype(float)

        mask = (srcs == int(node_idx)) | (dsts == int(node_idx))
        if not np.any(mask):
            return None

        neighbors = np.where(srcs[mask] == int(node_idx), dsts[mask], srcs[mask])
        if neighbors.size == 0:
            return None

        df = pd.DataFrame({
            'Neighbor': neighbors.astype(int),
            'Attention': att_scores[mask].astype(float)
        })
        df = df.groupby('Neighbor', as_index=False).mean()
        ...
        fig = px.bar(
            ...
            x='Neighbor',
            y='Attention',
            title=f'1-Hop {layer_name.upper()} Attention for Node {node_idx}',
            text_auto='.3f',
            labels={'Neighbor': 'Neighbor Node', 'Attention': 'Attention Score'},
            color='Attention',
...
def create_attention_panel(attention_weights, node_idx):
    ...
    if isinstance(attention_weights, dict):
        ...
        for layer_name in ('conv1', 'conv2'):
            layer_data = attention_weights.get(layer_name)
            if not isinstance(layer_data, (list, tuple)):
                continue
            ...
            avg_weight = float(np.mean(flat_values))
            max_weight = float(np.max(flat_values))
            summary_items.append(
                html.Div([
                    html.Div(layer_name.upper(), style={'fontWeight': 'bold', 'fontSize': '12px', 'color': '#555', 'marginBottom': '4px'}),
                    html.Div([
                        html.Span(f"Avg: {avg_weight:.4f}", style={'fontSize': '12px', 'color': '#007bff', 'fontWeight': 'bold'}),
                        html.Span(f" | Max: {max_weight:.4f}", style={'fontSize': '12px', 'color': '#666', 'marginLeft': '8px'})
                    ])
                ], style={'padding': '6px 0', 'borderBottom': '1px solid #eee'})
            )

        # Build a compact summary row for conv1 and conv2 and show side-by-side
        panel_children = [
            html.H5(f"GAT Attention for Node {node_idx}",
                   style={'marginBottom': '10px', 'color': '#333', 'fontSize': '14px', 'fontWeight': 'bold'}),
        ]

        if summary_items:
            # Ensure exactly two summary boxes (conv1, conv2) are shown side-by-side
            summary_boxes = []
            for item in summary_items:
                summary_boxes.append(html.Div(item, style={'flex': '1', 'minWidth': '160px', 'padding': '6px', 'border': '1px solid #eee', 'borderRadius': '4px', 'backgroundColor': '#fafafa'}))
            # If only one summary exists, add a placeholder for the other
            if len(summary_boxes) == 1:
                summary_boxes.append(html.Div(html.Div("No data"), style={'flex': '1', 'minWidth': '160px', 'padding': '6px'}))
            panel_children.append(html.Div(summary_boxes, style={'display': 'flex', 'gap': '12px', 'marginBottom': '12px'}))

        # Keep a single bar graph below. Prefer conv1, else conv2.
        graph = _build_attention_neighbor_bar(attention_weights, node_idx, layer_name='conv1')
        if graph is None:
            graph = _build_attention_neighbor_bar(attention_weights, node_idx, layer_name='conv2')

        if graph is not None:
            panel_children.append(graph)
        else:
            panel_children.append(html.Div("No 1-hop attention weights available for this node.", style={'color': '#999', 'fontStyle': 'italic', 'fontSize': '13px'}))
        ...
        dcc.Loading(id='loading-layer-3', type='circle', color='#007bff', children=[
            ...
            dcc.Tabs(
                ...
                children=[
                                ...
                                html.Div(style={
                                    ...
                                    html.Div(style={
                                        ...
                                        html.Div(style={'display': 'flex', 'alignItems': 'center', 'gap': '10px', 'marginBottom': '10px'}, children=[
                                            html.Span("Node Color: actual class", style={'fontWeight': 'bold', 'fontSize': '11px', 'color': '#555', 'marginBottom': '0'})
                                        ]),
                                        cyto.Cytoscape(
                                            id='graph-view',
                    ...
                    # ══════════════════════════════════════════════════
                    # TAB B: FULL GRAPH VIEW
                    # ══════════════════════════════════════════════════
                    dcc.Tab(
                        label='Full Graph View',
                        value='tab-full-graph',
                        style={
                            'fontWeight': 'bold', 'fontSize': '12px',
                            'padding': '8px 18px', 'letterSpacing': '0.3px',
                            'borderTop': '3px solid transparent',
                            ...
                            html.Div(style={
                                ...
                                html.Div(style={
                                    ...
                                    html.Div(style={'display': 'flex', 'alignItems': 'center', 'gap': '10px'}, children=[
                                        html.Label("Node Color", style={'fontWeight': 'bold', 'fontSize': '12px', 'color': '#333', 'marginBottom': '0'}),
                                        dcc.Dropdown(
                                            id='full-graph-color-mode',
                                            options=[
                                                {'label': 'Predicted class', 'value': 'predicted'},
                                                {'label': 'Actual class', 'value': 'actual'},
                                    ...
                                    html.Div(style={'display': 'flex', 'alignItems': 'center', 'gap': '10px'}, children=[
                                        html.Label("Filter by", style={'fontWeight': 'bold', 'fontSize': '12px', 'color': '#333', 'marginBottom': '0'}),
                                        dcc.Dropdown(
                                            id='full-graph-filter-metric',
                                            options=[
                                    ...
                                    html.Div(style={'minWidth': '280px', 'maxWidth': '380px'}, children=[
                                        html.Label("Filter range", style={'fontWeight': 'bold', 'fontSize': '12px', 'color': '#333', 'marginBottom': '6px', 'display': 'block'}),
                                        dcc.RangeSlider(
                                            id='full-graph-filter-range',
                                ...
                                dcc.Graph(
                                    id='full-graph-view',
                                    style={'width': '100%', 'height': '760px', 'minHeight': '760px'},
                                    config={
                                        'displayModeBar': True,
...
@callback(
    Output('full-graph-view', 'figure'),
    Input('layer3-tabs', 'value'),
    Input('full-graph-color-mode', 'value'),
    Input('full-graph-filter-metric', 'value'),
    Input('full-graph-filter-range', 'value'),
...
def update_full_graph_view(active_tab, color_mode, filter_metric, filter_range, dataset, graph_data, predictions_data, stability_results, selected_node):
    """
    Build a full-graph Plotly figure for the embedded frame.
    ...
    """
    MAX_MODAL_NODES = 800  # cap for the embedded graph
    ...
    try:
        ...
        true_labels = np.asarray(dataset.get('y')) if dataset.get('y') is not None else None
        has_true_labels = true_labels is not None and len(true_labels) == num_nodes and len(preds_arr) >= num_nodes

        def safe_int(arr, idx, default=-1):
            try:
                return int(arr[idx]) if arr is not None and 0 <= idx < len(arr) else default
            except Exception:
                return default

        # ── Apply confidence / stability filters ─────────────────────────
        filter_metric = (filter_metric or 'confidence').lower() if filter_metric else 'confidence'
        filter_range = filter_range if isinstance(filter_range, (list, tuple)) and len(filter_range) == 2 else [0.0, 1.0]
        try:
            filter_min, filter_max = float(filter_range[0]), float(filter_range[1])
        except Exception:
            filter_min, filter_max = 0.0, 1.0
        filter_min = max(0.0, min(filter_min, 1.0))
        filter_max = max(0.0, min(filter_max, 1.0))

        filtered_nodes = list(range(num_nodes))
        if filter_metric == 'confidence':
            confidences = predictions_data.get('confidences') if isinstance(predictions_data, dict) else None
            if confidences and len(confidences) >= num_nodes:
                filtered_nodes = [
                    n for n in filtered_nodes
                    if 0 <= n < len(confidences) and filter_min <= float(confidences[n]) <= filter_max
                ]
        elif filter_metric == 'stability' and stability_results:
            stability_map = {}
            for row in stability_results:
                try:
                    idx = int(row.get('node_idx'))
                    stability_map[idx] = float(row.get('stability'))
                except Exception:
                    continue
            if stability_map:
                filtered_nodes = [
                    n for n in filtered_nodes
                    if n in stability_map and filter_min <= stability_map[n] <= filter_max
                ]

        if len(filtered_nodes) == 0:
            return go.Figure(layout={
                'title': {'text': 'No nodes match the selected filter range.'},
                'paper_bgcolor': 'rgba(0,0,0,0)',
                'plot_bgcolor': '#f9fbff',
            })
        ...
        if len(filtered_nodes) > MAX_MODAL_NODES:
            sampled = True
            rng = np.random.default_rng(42)
            display_nodes = rng.choice(filtered_nodes, MAX_MODAL_NODES, replace=False).tolist()
        ...
        legend_title = 'Color Key'
        if color_mode == 'actual' and has_true_labels:
            node_value_map = {n: safe_int(true_labels, n, 0) for n in display_nodes}
            class_colors = [color_palette[c % len(color_palette)] for c in range(num_classes)]
            legend_name_fn = lambda cls: f'Class {cls}'
        elif color_mode == 'correctness' and has_true_labels:
            node_value_map = {n: 0 if safe_int(preds_arr, n, -1) == safe_int(true_labels, n, -2) else 1 for n in display_nodes}
            class_colors = ['#28a745', '#e74c3c']
            num_classes = 2
            legend_name_fn = lambda cls: 'Correct' if cls == 0 else 'Incorrect'
        else:
            node_value_map = {n: safe_int(preds_arr, n, 0) for n in display_nodes}
            class_colors = [color_palette[c % len(color_palette)] for c in range(num_classes)]
            legend_name_fn = lambda cls: f'Class {cls}'
        ...
        node_values = np.array([int(node_value_map.get(n, 0)) for n in G.nodes()])
        node_traces = []
        for cls in range(num_classes):
            cls_nodes = [n for n, val in zip(G.nodes(), node_values) if val == cls]
            if not cls_nodes:
                continue
            ...
            hover_templates = []
            for n in cls_nodes:
                pred_class = safe_int(preds_arr, n, -1)
                actual_class = None
                if true_labels is not None and 0 <= n < len(true_labels):
                    try:
                        actual_class = int(true_labels[n])
                    except Exception:
                        actual_class = None
                ...
                if actual_class is not None:
                    ...
                    hover_templates.append(
                        f"Node {n}<br>Predicted: {pred_class}<br>Actual: {actual_class}<br>{'Correct' if correct else 'Incorrect'}<extra></extra>"
                    )
                else:
                    hover_templates.append(
                        f'Node {n}<br>Predicted: {pred_class}<extra></extra>'
                    )
            ...
            node_traces.append(go.Scatter(
                ...
                name=f'{legend_title}: {legend_name_fn(cls)}',
            ))
...
def load_initial_data(dataset_name, model_type):
    ...
    try:
        ...
        if 'y' in dataset:
            try:
                true_labels = dataset['y'].cpu().tolist() if hasattr(dataset['y'], 'cpu') else list(dataset['y'])
            except Exception:
                try:
                    true_labels = list(dataset['y'])
                except Exception:
                    true_labels = None
        ...
        confidences = package.get('confidences', [])
        if not predictions:
            print("WARNING: No predictions found in package")

        initial_predictions = {'preds': predictions, 'confidences': confidences}
...
def update_bottom_layer(selected_link, selected_node, explanation, cpa_data, link_cpa_data, selected_path, analysis_mode, predictions_data, dataset, graph_data, package_meta, model_type, active_layer3_tab):
    try:
        empty_fig = go.Figure(layout={'title': "", 'paper_bgcolor':'rgba(0,0,0,0)', 'plot_bgcolor':'rgba(0,0,0,0)'})
        subgraph_color_mode = 'actual'
        ...
        color_palette = px.colors.qualitative.Plotly
        class_to_color_map = {i: color_palette[i % len(color_palette)] for i in range(num_classes)}
        true_labels = dataset.get('y') if isinstance(dataset, dict) else None
        yvals = true_labels
        ...
        if analysis_mode == 'link_prediction':
            ...
            mode_label = 'Predicted Class'
            if subgraph_color_mode == 'actual':
                mode_label = 'Actual Class'
            elif subgraph_color_mode == 'correctness':
                mode_label = 'Correctness'

            legend_content = html.Div([
                html.Div([
                    html.Span("Color Mode:", style={'fontWeight': 'bold', 'color': '#333'}),
                    html.Span(mode_label, style={'marginLeft': '5px', 'fontFamily': 'monospace', 'fontSize': '14px'})
                ], style={'marginBottom': '10px'}),
                html.Div([
            ...
            graph_elements, _ = create_subgraph_cytoscape(
                ...
                color_mode=subgraph_color_mode,
            ...
            try:
                ...
                if node_imp:
                    ...
                    try:
                        neighbor_fig.update_layout(title=f'Top Neighbor Importance')
                    except Exception:
                        pass
                else:
            ...
            try:
                ...
                feature_fig = empty_fig
            causal_content = None
            attention_content = None
            causal_style = {'display': 'none'}
            if active_cpa_data:
                causal_content = create_cpa_iv_panel(active_cpa_data, f"{u} \u2192 {v}")
            if model_type == 'GAT':
                attention_content = create_attention_panel(
                    get_attention_weights(package_meta),
                    f"{u} \u2192 {v}"
                )
            if causal_content or attention_content:
                explanation_children = []
                if causal_content:
                    explanation_children.append(html.Div(causal_content, style={'flex': '1', 'minWidth': '320px'}))
                if attention_content:
                    explanation_children.append(html.Div(attention_content, style={'flex': '1', 'minWidth': '320px'}))
                causal_content = html.Div(explanation_children, style={'display': 'flex', 'flexWrap': 'wrap', 'gap': '16px', 'alignItems': 'flex-start'})
        ...
        # B. NODE CLASSIFICATION MODE (Standard)
            ...
            if target_node_idx == -1:
                ...
                one_hop_neighbors = get_one_hop_neighbors(pyg_data, target_node_idx)
                if node_imp and len(node_imp) > 0 and one_hop_neighbors:
                    neighbor_imp = {n: node_imp.get(n, 0.0) for n in one_hop_neighbors}
                    neighbor_fig = plot_neighbor_importance(
                        neighbor_imp,
                        target_node_idx,
                        preds=preds,
                        y_labels=yvals,
                    ...
                    neighbor_fig = plot_neighbor_importance(
                        ...
                        y_labels=yvals,
                        one_hop_neighbors=one_hop_neighbors
                    ) if one_hop_neighbors else empty_fig
            ...
            if model_type == 'GAT':
                attention_content = create_attention_panel(
                    get_attention_weights(package_meta),
                    target_node_idx if target_node_idx != -1 else 'selected node'
                )
            if causal_content or attention_content:
                explanation_children = []
                if causal_content:
...
def evaluate_subgraph_predictions(link_pred_data, analysis_mode, package, dataset, model_type, dataset_name, graph_type):
    ...
    try:
        ...
        # Try to load subgraph model; GAT may reuse the same subgraph split as GCN
        subgraph_model_path = f"subgraph_models/{model_type}_{dataset_name}_subgraph.pkl"
        fallback_model_path = f"subgraph_models/GCN_{dataset_name}_subgraph.pkl"
        ...
        try:
            ...
            if not os.path.exists(subgraph_model_path):
                if os.path.exists(fallback_model_path):
                    print(f"WARNING: Missing subgraph model for {model_type}. Falling back to GCN split: {fallback_model_path}")
                    subgraph_model_path = fallback_model_path
                else:
...
def render_drilldown_scatter(click_data, epsilon, plot_type, results):
    ...
    # Normalize link-oriented results to node-oriented schema if necessary
    if 'node_idx' not in df.columns and 'source' in df.columns and 'target' in df.columns:
        try:
            df['node_idx'] = df['source'].astype(str) + '-' + df['target'].astype(str)
            if 'degree' not in df.columns and 'degree_sum' in df.columns:
    ...
    # After normalization, ensure required plotting columns exist; fill with defaults if missing
